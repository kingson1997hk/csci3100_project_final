...
<template>
  <div id="big-form">
    <ul id="bar-ul">
      ...
      <li class="bar-li" style="float:right"><h3  class="bar-h3"><router-link to="/" class="bar-item">User: {{this.$parent.name}}<br>Log Out</router-link></h3></li>
    </ul>
    <div id="small-item">
    <h1>{{ msg }}</h1>
    <h2>You can report user here or submit the application of cancelling a gathering with an appropriate reason.</h2>
    <form @submit.prevent="reportuser">
     <label for="userid">Reported User / Room ID: <br></label>
    <input v-model="userid" placeholder="UserID / Room ID" required><br>
      <br>

     <label for="reportreason">Report Reason / Cancelling Reason: </label> <br>
    <textarea v-model="reportreason" placeholder="" required></textarea>
    <br>
      <br>
